...
| AUDIT-008 | P1 | Vendor | Vendor Dashboard | `VendorAnalytics.tsx`'s revenue/bookings/customer/top-listings charts are 100% hardcoded mock numbers (₹1,25,000 revenue, 89 bookings, "Mountain View Room" ₹87,500, etc.) — not fed by any real API call. Currently dead code (never imported anywhere), so no live user sees it, but it's a fabrication landmine the moment someone wires it into the dashboard. No real per-vendor revenue/booking aggregation endpoint exists yet (booking-completion → Settlement isn't wired up; the only backend vendor-analytics route, `GET /admin/vendor-analytics`, is admin-only and platform-wide). | **Fixed** — see FIXED LOG |
...
**AUDIT-008** — `VendorAnalytics.tsx` is dead code (confirmed unimported anywhere in the app both times it was touched this session), but its hardcoded revenue/bookings/customer/top-listings mock data was a fabrication landmine waiting for the first person to wire it into the dashboard. Confirmed no real per-vendor data source exists to replace it with: booking-completion never transitions bookings to `COMPLETED` and no `Settlement` rows are ever created (see CLAUDE.md §3 "What's missing"), and the only backend vendor-analytics route (`GET /admin/vendor-analytics`) is `@Roles(Role.Admin)`-gated and platform-wide, not scoped to one vendor. Replaced the fabricated revenue/bookings/customer-geography/top-listings charts with an honest "Coming soon" empty state, matching the pattern the file's own "Protocol Health Trace" section already used for its own not-yet-real metrics. No `/chrome` re-verification possible (still unreachable dead code — not linked from any route); verified via `tsc --noEmit` and reading the rendered JSX only.
